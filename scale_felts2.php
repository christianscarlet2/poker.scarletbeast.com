...
$emoji = ['🤖','👾','🦾','🧠','⚙️','🔩','🛸','🦿','🔮','🐉','♠','♥','♦','♣','🔴','🟢'];
$i = 1;
while (User::where('is_bot', true)->count() < 160) {
    if ($i > 999) break;
...
echo "machine_only NLHE tables: " . PokerTable::where('table_type','machine_only')->where('status','active')->where('game_type','nlhe')->count() . "\n";
